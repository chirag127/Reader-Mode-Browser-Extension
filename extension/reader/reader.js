...
function initializeReader() {
  ...
  makeDraggable(document.getElementById('tts-controls'), document.getElementById('tts-drag-handle'));
}
...
function applySettings() {
  ...
  document.getElementById('theme-toggle').textContent = currentSettings.theme === 'dark' ? '☀️' : '🌙';
  
  // Apply font settings
  document.getElementById('article-content').style.fontFamily = currentSettings.fontFamily;
  document.getElementById('article-content').style.fontSize = `${currentSettings.fontSize}px`;
  
  // Apply speech settings
  document.getElementById('tts-speed').value = currentSettings.speechRate.toString();
  
  // Apply voice selection if available
  if (currentSettings.voiceURI) {
    const voiceSelect = document.getElementById('tts-voice');
    if (voiceSelect.querySelector(`option[value="${currentSettings.voiceURI}"]`)) {
...
function setupEventListeners() {
  ...
  // TTS controls
  document.getElementById('tts-play-pause').addEventListener('click', togglePlayPause);
...
async function extractContent(url) {
  try {
    ...
    document.getElementById('article-content').innerHTML = '<p>Loading content...</p>';
    ...
    // Set the title
    document.getElementById('article-title').textContent = response.content.title;
...
function formatContent(content) {
  ...
  formatted = formatted.replace(/\[IMAGE\]/g, '<div class="image-placeholder">[Image]</div>');
...
function changeFontSize(delta) {
  currentSettings.fontSize = Math.max(12, Math.min(32, currentSettings.fontSize + delta));
  document.getElementById('article-content').style.fontSize = `${currentSettings.fontSize}px`;
  saveSettings();
}
...
function showError(message) {
  ...
  document.getElementById('article-content').innerHTML = `<p class="error-message">${message}</p>`;
}
